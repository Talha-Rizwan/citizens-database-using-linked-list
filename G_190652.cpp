#include "pch.h"
#include"C:\Users\talha\Desktop\assi_02\Project1\Header.h"
...
TEST(alian_declare, CBID) {

    Database<string> db("CBID.txt", "CCID.txt", "CCND.txt");

    ASSERT_TRUE(db.declareAlien(6488) == 1);

    ASSERT_TRUE(db.declareAlien(4444) == 0);

}

TEST(alian_check, CBID) {

    Database<string> db("CBID.txt", "CCID.txt", "CCND.txt");

    ASSERT_TRUE(db.verifyAlienDBUpdate(6488) == 1);
